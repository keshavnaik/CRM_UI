...
angular.module('ngS3upload.config', []).
  ...
  service('S3Uploader', ['$http', '$q', '$window', '$rootScope', function ($http, $q, $window,$rootScope) {
    this.uploads = 0;
    var self = this;
    ...
    this.upload = function (scope, uri, key, acl, type, accessKey, policy, signature, file) {
      //console.log(file);
      //$rootScope.selectedFileName = file.name;
      if(uri === 'https://lssaascrm.s3.amazonaws.com/'){
     if(file.name.substr(-4) !== '.csv') {
       $window.alert('Only .csv  file format is allowed');
       file = null;
     }
   }
