...
$scope.getFeedbackFilter = function (status) {
    ...
    $scope.adminCallerStatsByDate = function(selFromDate,selToDate){
      ...
      selectedFromDate = selectedFromDateConversion;
      selectedToDate = selectedToDateConversion;
      getCrePsfDashboardData(selectedCaller,selectedBranch,adminUserScId);
      getFeedbackYesNoStats();
      getFeedbackRatingStats();
    }
...
$scope.getSelectedCaller = function (selCaller) {
 ...
 function getCrePsfDashboardData(selCaller,selBranch,id) {
      ...
      showLoader(true);
      var lsToken = TokenService.getToken(adminUserScId);
      PSFPerformanceDashboardStatsDataService.query({fromDate:selectedFromDateConversion,toDate:selectedToDateConversion,scId:adminUserScId,callerId:selectedCaller,branchId:selectedBranch,token:lsToken},function(data) {
...
$scope.getPericularStatsData = function (dataTypeName,status) {
      ...
      if(reportTypeForCustomerDetails === 'export'){
        $scope.exportStatus = '';
         $window.open(serviceURL+'get_admin_dashboard_psf_details/'+selectedFromDateConversion+'/'+selectedToDateConversion+'/'+adminUserScId+'/'+selectedCaller+'/'+selectedBranch+'/'+selectedStatsStatus+'/'+reportTypeForCustomerDetails+'/'+lsToken);
          showLoader(false);
          reportTypeForCustomerDetails = 'submit';
       }else if(reportTypeForCustomerDetails === 'submit'){
      PSFPerformancesStatsDetailedService.query({fromDate:selectedFromDateConversion,toDate:selectedToDateConversion,scId:adminUserScId,callerId:selectedCaller,branchId:selectedBranch,status:selectedStatsStatus,reportType:reportTypeForCustomerDetails,token:lsToken}, function(data) {
...
var feedBackCount15 = [];
     $scope.loaderIconFeedBack = true;
      ...
      var lsToken = TokenService.getToken(adminUserScId);
       GetFeedbackYesNoStatsService.query({fromDate:selectedFromDate,toDate:selectedToDate,scId:adminUserScId,caller:selectedCaller,branch:selectedBranch,token:lsToken},function (data) {
       console.log('FeedBack Yes No Stats :',data);
       feedBackStats = data;
...
$scope.feedBackDataChart.push($scope.feedBackDataChart15);
    ...
    function getFeedbackYesNoCustomerDetails(selFeedbackId,selStatus,selFeedbackStatus,selExportType) {
      ...
      var lsToken = TokenService.getToken(adminUserScId);
       if(reportTypeForCustomerDetails === 'export'){
          $window.open(serviceURL+'get_service_feedback_details/'+selectedFromDate+'/'+selectedToDate+'/'+adminUserScId+'/'+selectedCaller+'/'+selectedBranch+'/'+selFeedbackId+'/'+selectedStatsStatus+'/'+selFeedbackStatus+'/'+reportTypeForCustomerDetails+'/'+lsToken);
          console.log(serviceURL+'get_service_feedback_details/'+selectedFromDate+'/'+selectedToDate+'/'+adminUserScId+'/'+selectedCaller+'/'+selectedBranch+'/'+selFeedbackId+'/'+selectedStatsStatus+'/'+selFeedbackStatus+'/'+reportTypeForCustomerDetails+'/'+lsToken);
          showLoader(false);
          reportTypeForCustomerDetails = 'submit';
      }
      else if (reportTypeForCustomerDetails === 'submit'){
...
var feedBackRatingCount10 = [];
     ...
     $scope.feedBackRatingDataErrorMsg = '';
      var lsToken = TokenService.getToken(adminUserScId);
       GetFeedbackRatingStatsService.query({fromDate:selectedFromDate,toDate:selectedToDate,scId:adminUserScId,caller:selectedCaller,branch:selectedBranch,token:lsToken},function (data) {
       console.log('FeedBack Rating Stats :',data);
...
$scope.exportFeedbackCustomerDetails = function(selExportStatus){
    console.log('Hi',selExportStatus);
    if(!defaultDetails){
...
var feedBackCountDefault2 = [];
      ...
      var lsToken = TokenService.getToken(adminUserScId);
       GetServiceFeedbackYesNoDefaultStatsService.query({fromDate:selectedFromDate,toDate:selectedToDate,scId:adminUserScId,caller:selectedCaller,branch:selectedBranch,token:lsToken},function (data) {
